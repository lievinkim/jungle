...
// RB-DELETE-FIXUP(T, x)
    ...
    // 삭제한 노드가 블랙 노드인 경우
        // 삭제한 위치에 들어온 노드에 extra-black 생성
        // red&black 상태가 되면 해당 노드를 black으로 만들고 종료
        // black&black인 경우 트리의 위쪽으로 계속 전달
        // (최악의 경우) 루트까지 올라가는 경우 그냥 extra-black 삭제

        // Loop Invariant
        // x는 루트가 아닌 double-black 노드
        // w는 x의 형제 노드
        // w는 NIL 노드가 될 수 없음 (그렇지 않으면 x의 부모에 대해 조건 5가 위반)

        // 경우 1, 2, 3, 4 : x가 자기 부모의 왼쪽 자식인 경우
        // 경우 5, 6, 7, 8 : x가 자기 부모의 오른쪽 자식인 경우
